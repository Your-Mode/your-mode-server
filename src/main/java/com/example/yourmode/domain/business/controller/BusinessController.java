...
import com.example.yourmode.domain.business.service.BusinessService;
...
public class BusinessController {
    ...
    public BaseResponse<BusinessListResponse> getBusinessList(
        ...
        return BaseResponse.onSuccess(businessService.getBusinessList(pageable));
    }

    // 업장 상세 정보 조회
    @Operation(summary = "업장 상세 정보 조회", description = "특정 업장의 상세 정보를 조회함")
    @GetMapping("/{businessId}")
    public BaseResponse<BusinessInfoResponse> getBusinessDetail(
            @PathVariable Long businessId
    ) {
        return BaseResponse.onSuccess(businessService.getBusinessDetail(businessId));
    }
}
